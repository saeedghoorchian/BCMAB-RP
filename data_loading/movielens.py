...
def get_movielens_data():
    ...
    streaming_batch = pd.read_csv(f"{PROJECT_DIR}/dataset/movielens/streaming_batch.csv", sep="\t", header=0, engine="c")
    user_feature = pd.read_csv(f"{PROJECT_DIR}/dataset/movielens/user_feature.csv", sep="\t", header=0, index_col=0, engine="c")
    actions_id = list(pd.read_csv(f"{PROJECT_DIR}/dataset/movielens/actions.csv", sep="\t", header=0, engine="c")["movie_id"])
