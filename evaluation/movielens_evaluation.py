...
def evaluate_policy_on_movielens(policy, times, user_stream, user_feature, actions, action_features, reward_list):
    ...
    if policy.name != 'random':
        ...
        while t < times:  #
            j = j + 1  #
            if len(np.array(user_feature[user_feature.index == user_stream.iloc[j, 0]])) == 0:  #
                # print("time with no user:" + str(j))
                continue
            feature = np.array(user_feature[user_feature.index == user_stream.iloc[j, 0]])[0]  #
            full_context = {}
            for action_id in actions:  # movie_id :)
            ...
            watched_list = reward_list[reward_list['user_id'] == user_stream.iloc[j, 0]]  #
